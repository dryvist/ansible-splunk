...
exempt_indexes = {det["index"] for det in DEFAULTS["splunk_docker_silence_exemptions"] if det.get("exempt")}
...
if "exempt_idx" in exempt_gate_fired:
    errors.append("FAIL: regression fixture -- exempt_idx fired despite the exempt=0 gate")
if "real_idx" not in exempt_gate_fired:
    errors.append("FAIL: regression fixture -- real_idx (non-exempt, fully silent) did not fire")
...
if "exempt_idx" not in {row["index"] for row in simulate(THRESHOLD_ONLY_SEARCH)}:
    errors.append(
        ...
        "still fire on a never-ingested index (proving the mechanism this replaced "
